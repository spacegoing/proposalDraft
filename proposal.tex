...
\begin{document}
	...
	\subsection{Theoretical Background}
		...
		Gould\cite{gouldlearning} reparameterized the potential by sampling $K$ break points uniformly distributed on $[0,1]$. Let linear parameter vector $\boldsymbol{\theta}\in\mathbb{R}^K$ be the sampled values, the energy function satisfying all propositions in \cite{gouldlearning} can be writen as  
		\begin{equation*}
		\theta_k = \left\{
		\begin{aligned}
		& b_1	& \text{for} \ k=0\\
		...
		\right.
		\end{equation*}
		where $\boldsymbol{\phi}(\mathbf{y})\in \mathbb{R}^K$ is the correspondin feature map. Thus we can rewrite the Equation 1 into a linear form
		\begin{align}
	...
	+++++++++++
	Other extensions may including deep infrastructure and improve loss function and parallel computing
